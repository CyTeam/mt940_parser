...
class MT940
  ...
  class AccountBalance < Field
    ...
    def parse_content(content)
      ...
      amount_str = Regexp.last_match(4).tr(',', '.')
  ...
  class StatementLine < Field
    ...
    def parse_content(content)
      ...
      @funds_code = case Regexp.last_match(3)
                    when 'C'
                      :credit
                    when 'D'
                      :debit
  ...
  class StatementLineInformation < Field
    ...
    def parse_content(content)
      ...
      if seperator = Regexp.last_match(3)
        sub_fields = Regexp.last_match(2).scan(/#{Regexp.escape(seperator)}(\d{2})([^#{Regexp.escape(seperator)}]*)/)
