...
# $DEBUG = true
class TestCustomerStatementMessage < Minitest::Test
  def setup
  ...
  def test_parsing_a_file_with_broken_structure_should_raise_an_exception
    ...
    assert_raises(StandardError) { MT940::CustomerStatementMessage.parse_file(file) }
  end
  ...
  def test_should_raise_method_missing_when_asking_statement_lines_for_unknown_stuff
    ...
    assert_raises(NoMethodError) { message.statement_lines.first.foo }
  end
end
